...
class PolicyExtractionInput(BaseModel):
    ...
    known_vendors: list[Annotated[str, Field(max_length=128)]] = Field(default_factory=list, max_length=50)
    known_categories: list[Annotated[str, Field(max_length=64)]] = Field(default_factory=list, max_length=30)
...
class Transaction(BaseModel):
    ...
    daily_cumulative: Optional[float] = None
...
class ConnectedAccountCreate(BaseModel):
    ...
    external_account_id: Optional[str] = Field(None, max_length=255)
...
class EvaluateRequest(BaseModel):
    ...
    external_txn_id: Optional[str] = Field(None, max_length=255)
    payee: str = Field("", max_length=255)
    amount: float = Field(..., ge=0.0001, le=1_000_000_000)
    currency: str = Field("USD", max_length=3)
    category: Optional[str] = Field(None, max_length=64)
    vendor_status: Optional[str] = Field(None, max_length=64)
    vendor_age_days: Optional[int] = None
    daily_cumulative: Optional[float] = Field(None, ge=0)
    initiated_by: str = "agent"
    txn_metadata: Optional[dict] = None
...
class PaymentEventResponse(BaseModel):
    ...
    matched_policy_id: Optional[UUID] = None
    decision_rationale: Optional[dict] = None
