...
Also extends the existing `approvals` table:
  ...
  - approvals_target_ck extended to include 'payment_event'
...
def upgrade() -> None:
    ...
    # Extend the target CHECK constraint to include 'payment_event'.
...
def downgrade() -> None:
    ...
    # Restore approvals target constraint and remove guardrails extension.
    op.drop_constraint("ck_approvals_target", "approvals", type_="check")
    op.create_check_constraint(
        "ck_approvals_target",
        "approvals",
        "target IN ('recommendation','package')",
    )
